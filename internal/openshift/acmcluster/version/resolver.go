...
// Resolver resolves a K8s minor version to a release image by looking up
// ClusterImageSets via the compatibility matrix.
type Resolver struct {
	client client.Client
	matrix CompatibilityMatrix
}

// NewResolver creates a Resolver with the given compatibility matrix.
func NewResolver(c client.Client, matrix CompatibilityMatrix) *Resolver {
	return &Resolver{
		client: c,
		matrix: matrix,
	}
...
func (r *Resolver) Resolve(ctx context.Context, k8sVersion string) (string, error) {
	...
	for _, item := range cisList.Items {
		...
		if ExtractOCPVersion(releaseImage) == ocpVersion {
...
func (r *Resolver) k8sToOCP(k8sVersion string) (string, error) {
...
func ReleaseImageToK8sVersion(releaseImage string, matrix CompatibilityMatrix) string {
	ocpMinor := ExtractOCPVersion(releaseImage)
	if ocpMinor == "" {
		return ""
	}
