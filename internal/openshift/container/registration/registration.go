...
func (r *Registrar) register(ctx context.Context, provider dcmv1alpha1.Provider) error {
	// The DCM server uses the provider name field for deduplication, so
	// params (the first argument after ctx) is nil — no query parameters
	// are needed for the create-or-update semantic.
